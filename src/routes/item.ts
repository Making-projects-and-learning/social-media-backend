...
router.post(
  ...
  [
    ...
    check("color", '"color" cant be higher than 16 length.').isLength({
      max: 16,
    }),
    ...
    check("description", '"description" must have at least 6 length.').isLength(
      { min: 6 }
    ),
    check(
      "description",
      '"description" cant be higher than 60 length.'
    ).isLength({ max: 60 }),
    ...
    check("name", '"name" cant be higher than 30 length.').isLength({
      max: 30,
    }),
